...
try {
	const { images_optimization } = require(path.join(
		process.cwd(),
		"cms",
		"_data",
		"settings",
...
module.exports = function (eleventyConfig) {
	...
	eleventyConfig.addShortcode(
		"image",
		async function (src, alt = "", dataSizes = "", attributes = "") {
			if (!src) {
				return "";
			}
			...
			try {
				let metadata = await Image(src, {
					...
					urlPath:
						"https://development.nanproperties.com/assets/images/",
		...
		for (let key in seo) {
			switch (key) {
				case "noindex":
					...
					break;
				case "title":
					seoString += `<title>${htmlEntities(
						seo.title
					)}</title><meta property="og:title" content="${htmlEntities(
						seo.title
					)}" />`;
					break;
				case "description":
					seoString += `<meta name="description" content="${htmlEntities(
						seo.description
					)}">`;
					break;
				default: {
					if (key == "additional_tags") {
						...
						seoString += `<meta property="${escape(
							key
						)}" content="${htmlEntities(seo[key])}">`;
					} else {
						seoString += `<meta name="${escape(
							key
						)}" content="${htmlEntities(seo[key])}">`;
					}
					break;
	...
	eleventyConfig.addShortcode("money", async function (price) {
		...
		return (
			await Promise.all(
				ecommerceFormat.currencies.map(async (currency) => {
					const currentPrice = !!price[currency.currencyCode]
						? formatPrice(price[currency.currencyCode], currency)
						: formatPrice(
								await convertPrice(
									basePrice,
